...
impl ErrCodeGen {
    pub fn new(errcodes: ErrCodes) -> Self {
        Self { errcodes }
    }

    pub fn codegen<W: std::io::Write>(self, writer: &mut W) -> Result<()> {
        writer.write_all(HEADLINE)?;

        for ErrCode { sqlstate, .. } in self.errcodes.iter().flat_map(|e|e.1) {
            write!(
                writer,
        ...
        for ErrCode { sqlstate, errcode_macro_name, .. } in self.errcodes.iter().flat_map(|e|e.1) {
