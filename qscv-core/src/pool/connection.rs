...
use crate::{
    connection::Connection, database::Database, error::Error,
    pool::options::PoolConnectionMetadata, sync::AsyncSemaphoreReleaser,
};
    ...
    async fn return_to_pool(mut self) -> bool {
        ...
        if let Some(test) = &self.guard.pool.options.after_release {
            ...
            match (test)(&mut self.inner.raw, meta).await {
                ...
                Err(error) => {
                    log::warn!("error from `after_release`: {error}");
        ...
        if let Err(error) = self.raw.ping().await {
            log::warn!("error occurred while testing the connection on-release: {error}");
...
impl<DB: Database> Floating<DB, Idle<DB>> {
    ...
    pub async fn close(self) -> DecrementSizeGuard<DB> {
        if let Err(error) = self.inner.live.raw.close().await {
            log::debug!("error occurred while closing the pool connection: {error}");
        }
        self.guard
    }
